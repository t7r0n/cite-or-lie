...
COMPANY = 'Mimos'
REPO = 'cite-or-lie'
PROJECT_TERMS = ['mimos', 'hidden', 'cites', 'domain', 'might', 'citing', 'retrieval', 'surfaced']
PROJECT_METRICS = ['mimos_coverage', 'hidden_risk', 'cites_precision', 'domain_latency']
PROJECT_FAILURES = ['mimos_drift', 'hidden_gap', 'cites_misroute', 'domain_blindspot']
PROJECT_ARCHETYPES = [{'name': 'mimos evidence replay', 'trigger': 'mimos signal changes while hidden context is stale', 'expected': 'block release until cited evidence is regenerated'}, {'name': 'hidden boundary probe', 'trigger': 'hidden handoff crosses a policy or trust boundary', 'expected': 'route to reviewer with evidence packet'}, {'name': 'cites regression harness', 'trigger': 'cites behavior regresses against the last accepted fixture', 'expected': 'open a regression issue with trace and benchmark delta'}, {'name': 'domain operator packet', 'trigger': 'domain output needs a human-readable audit packet', 'expected': 'accept only if decision claims cite fixture evidence'}]
PROJECT_DIRECTION = "A grounded citation auditor for AI answer engines: every claimed citation in a Mimos report is verified against the answer engine's own retrieval trace (when available) and a deterministic re prompt protocol, returning a per citation (grounded | memorized | hallucinated) verdict with confidence — the compliance substrate AEO has been missing."
...
HERO_TITLE = 'Mimos Evidence Triage Board'
